...
export type Event = {
  ...
  duration: number;
  state: Omit<(typeof EventModel.state.enumValues)[number], "completed">;
  start: Date;
  paused: Date;
};
...
async function getEvent(userId: number, date: Date): Promise<Event> {
  await colosOldEvents({ userId, date });
...
`,
    ...
    .where(
      and(
        ...
        or(gte(EventModel.createdAt, date), lt(EventModel.createdAt, date)),
      ),
    )
...
async function createEvent({
  ...
  await db.insert(EventModel).values({
    ...
    userId,
    state: "paused",
    end: new Date(new Date().getTime() + addedTimeInMiliSeconds),
  });
}

async function colosOldEvents({
  userId,
  date,
}: {
  userId: number;
